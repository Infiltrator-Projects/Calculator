...
Calculator 0.2.0 also adds a native **Tools** workbench on Linux, Windows and iPhone. The shared C++ implementation provides twelve domain families: engineering/electrical calculations, general unit conversion, ICT/network subnet and transfer calculations, storage/filesystem capacity and allocation calculations, date/time arithmetic, the broader constants catalogue, descriptive statistics, native graphing, real equation solving, exact-decimal/rational arithmetic, arbitrary-precision integer arithmetic and complex-number arithmetic. Linux and Windows render the graph natively through Cairo/GDI; iPhone renders the same shared sample points with SwiftUI Canvas. Tool syntax and domain contracts are documented in [docs/TOOLS.md](docs/TOOLS.md).
