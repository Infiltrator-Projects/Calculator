...
## Mint replacement target completed

The final required Linux Mint / GNOME Calculator 41.1 replacement item is implemented: Scientific mode now has an integrated arbitrary-precision real/complex value domain in the shared C++ core.

High-precision decimal components flow through expression evaluation, variables, Session state, Controller state and platform-neutral result models without binary64 truncation. Arithmetic and supported transcendental/complex operations use the shared multiprecision backend; Linux, Windows and iPhone render the same semantics. Independent MPFR/MPC oracle tests provide release-CI evidence for representative huge, tiny, transcendental and complex cases.

Currency conversion remains an explicit non-goal and does not block replacement completeness.

## Continuing priorities

- maintain and extend regression evidence for the integrated arbitrary-precision real/complex Scientific value domain
