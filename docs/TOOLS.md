...
Calculator exposes fourteen extended calculation families through the **Tools** button on Linux, Windows and iPhone. The workbench is native on each platform, but the catalogue, prompts, examples, evaluation rules and graph samples come from one shared C++ implementation in `src/core/advanced_tools.*`.
